...
m.eb = Param() # void fraction
m.CF = Param(m.Data, m.Comp, initialize = CFeed) # feed concentration
m.K = Var(m.Comp, within=PositiveReals) # Henry constant
m.Kap = Var(m.Comp, within=PositiveReals) # mass transfer coefficient
m.b = Var(m.Comp, within=PositiveReals) # anti-langmuir parameter
...
m.A = Param(m.CP, m.CP) # parameters of collocation points
...
data.load(filename = 'data_ParamEst.dat')
...
def NonLinearEquilibrium_rule(m, d,i,j,k,l):
...
m.NonLinearEquilibrium=Constraint(m.Data, m.Comp, m.Col, m.t, m.x, rule=NonLinearEquilibrium_rule)
