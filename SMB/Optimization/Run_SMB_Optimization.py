...
dirname00 = f"Optimization_Result_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}/"
...
if DScheme == 'CENTRAL':
    ...
    if Axial_D == True:
        def C_Axial2ndDerivativeConstraintEnd_rule(m,i,j,k):
            return m.dCdx2[i,j,k,value(m.L)]==(2*m.C[i,j,k,value(m.L)]-5*m.C[i,j,k,m.x[Nfex-1]]+4*m.C[i,j,k,m.x[Nfex-2]]-m.C[i,j,k,m.x[Nfex-3]])/((m.L/(Nfex-1))**3) 
        instance.C_Axial2ndDerivativeConstraintEnd=Constraint(instance.Comp, instance.Col, instance.t,  rule=C_Axial2ndDerivativeConstraintEnd_rule)
...
if ini_plots == True:
    ...
    filename = dirname0ini + f"concentration_profile"
...
if DV == True:
    print("Dead Volume = %s " %(value(instance.LRe)))
...
if PowerFeed == True:
    dirname03 = dirname00 + f"PowerFeed_Flow_Rates_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}/"
    os.makedirs(dirname03, exist_ok=True)
    
    dirname04 = dirname00 + f"Internal_Flow_Rates_Profile_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}/"
    ...
    for i in range(NZone):
        plt.figure()
        plt.step(t, U_four[i], where='pre', color=U_color[i])
        plt.scatter(t, U_four[i], marker='*', color=U_color[i], label=U_name[i])
        plt.legend()
    ...
    for i in range(Colum):
        plt.figure()
        plt.step(t, U_Col[i], where='pre', color='k')
        plt.scatter(t, U_Col[i], marker='*', color='k')
        plt.xlim(0, value(instance.StepTime))
